...
class HashMap<_KEY,zval*>: public Map<_KEY,zval*> {
	friend class HashMapIterator<_KEY,zval*>;
public:
	typedef HashMapIterator<_KEY,zval*> iterator;

	HashMap(){
		count=0;
		bucket_count=8;
		buckets = new HashMapEntry<_KEY, zval*>*[bucket_count]();
		internalIteratorStart = nullptr;
		internalIteratorEnd = nullptr;
	}
	...
	bool containsKey(const _KEY& key) const{
		...
		HashMapEntry<_KEY, zval*>* currentBucket = buckets[bucketNumber];
		while(currentBucket!=nullptr) {
			if(currentBucket->hash == hashValue && keyComparator(currentBucket->data.key,key)==0) {
				return true;
			}
	...
	bool containsValue(zval*& value) const{
		if(count==0) return false;
		for(std::size_t i=0; i< bucket_count; ++i) {
			HashMapEntry<_KEY, zval*>* currentBucket = buckets[i];
			while(currentBucket!=nullptr) {
				if(valueComparator(currentBucket->data.value, value)==0) {
					return true;
				}
	...
	void set(const _KEY& key, zval*& value){
		...
		if(currentBucket!=nullptr) {
			...
			while(currentBucket!=nullptr) {
				...
				if(currentBucket->next == nullptr) {
					// append to end
					HashMapEntry<_KEY, zval*>* be = new HashMapEntry<_KEY, zval*>;
					be->hash = hashValue;
					be->data.key = key;
					be->data.value = value;
					be->next = nullptr;
...
private:
	...
	HashMapEntry<_KEY, zval*>* recursiveDeleteKey(HashMapEntry<_KEY, zval*>* currentBucket, const _KEY& key) {
		if (currentBucket == nullptr)
			return nullptr;

		if (keyComparator(currentBucket->data.key, key)==0) {
			HashMapEntry<_KEY, zval*>* tempNextP;
			tempNextP = currentBucket->next;
			zval_ptr_dtor(&currentBucket->data.value);
			delete currentBucket;
			-- count;
	...
	HashMapEntry<_KEY, zval*>* recursiveDeleteValue(HashMapEntry<_KEY, zval*> *currP, zval*& value) {
		if (currP == nullptr)
			return nullptr;
		...
		if (valueComparator(currP->data.value,value)==0) {
			HashMapEntry<_KEY, zval*>* tempNextP;
			tempNextP = currP->next;
			zval_ptr_dtor(&currP->data.value);
			delete currP;
			-- count;
			return tempNextP;
		}
	...
	void emptyBuckets() {
		for(std::size_t i=0; i< bucket_count; ++i) {
			HashMapEntry<_KEY, zval*>* head = buckets[i];
			HashMapEntry<_KEY, zval*>* del = head;
			while(del != nullptr) {
				head = head->next;
	...
	void rehash(){
		...
		HashMapEntry<_KEY, zval*>** new_buckets = new HashMapEntry<_KEY, zval*>*[new_bucket_count]();
		...
		for(std::size_t i=0; i< bucket_count; ++i) {
			...
			while(currentBucket!=nullptr) {
	            ...
	            // advance n *before* moving node to new hash bed
				HashMapEntry<_KEY, zval*>* tmp = currentBucket;
	            currentBucket = currentBucket->next;

	            // find the proper collision list pointer.
	            HashMapEntry<_KEY, zval*>*& bucket = new_buckets[bucket_number];
	            tmp->next = bucket;
	            bucket = tmp;
			}
		}
	...
	std::size_t count;
	std::size_t bucket_count;
	HashMapEntry<_KEY, zval*> **buckets;
